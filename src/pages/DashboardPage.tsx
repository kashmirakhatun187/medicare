  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <section className="dashboard-hero relative overflow-hidden rounded-3xl p-5 sm:p-7 text-white">
        <div className="absolute -right-16 -top-20 h-64 w-64 rounded-full bg-white/10 blur-2xl floating-orb" />
        <div className="absolute -bottom-24 left-1/3 h-52 w-52 rounded-full bg-cyan-300/20 blur-3xl floating-orb-delayed" />
        <div className="absolute top-10 right-1/4 h-32 w-32 rounded-full bg-amber-300/10 blur-2xl" />
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-cyan-50 backdrop-blur-sm">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {['Appointments', 'Emergency', 'Pharmacy', 'Lab'].map((tag, i) => (
                <span key={tag} className={`px-3 py-1 rounded-full text-xs font-medium bg-white/10 border border-white/10 backdrop-blur-sm hover:bg-white/20 transition-all cursor-pointer stagger-in stagger-${i + 1}`} onClick={() => onNavigate?.(['appointments','emergency','pharmacy','lab'][i] as PageId)}>{tag}</span>
              ))}
            </div>
          </div>
          ...
          {visibleModules.map((module, index) => {
            ...
            return (
              <button key={`${module.label}-${index}`} onClick={() => onNavigate?.(module.page)} className="module-tile group text-left" title={module.description}>
                <span className={`module-icon bg-gradient-to-br ${module.tone}`}><Icon size={20} strokeWidth={1.8} /></span>
                <span className="mt-2 line-clamp-2 text-center text-[10px] font-bold uppercase leading-3 tracking-wide text-slate-600 transition-colors group-hover:text-brand-700">{module.label}</span>
                <span className={`mt-1.5 h-1 w-6 rounded-full bg-gradient-to-r ${module.tone} opacity-60 transition-all group-hover:w-10`} />
              </button>
            );
          })}
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="card-premium p-6 lg:col-span-2">
          ...
          <div className="flex h-48 items-end justify-between gap-3">
            {revenueByDay.map((day, i) => <div key={day.day} className="flex h-full flex-1 flex-col items-center gap-2 group">
              <div className="flex h-full w-full items-end rounded-t-xl bg-slate-100/80 relative overflow-hidden">
                <div className="rev-bar w-full rounded-t-xl bg-gradient-to-t from-brand-600 via-cyan-500 to-sky-400 transition-all duration-300 hover:brightness-110 group-hover:from-brand-700 group-hover:via-cyan-600 group-hover:to-sky-500" style={{ '--bar-height': `${Math.max((day.amount / maxRevenue) * 100, 3)}%`, height: `${Math.max((day.amount / maxRevenue) * 100, 3)}%`, animationDelay: `${i * 0.08}s` } as React.CSSProperties} />
              </div>
              <span className="text-xs font-medium text-slate-500">{day.day}</span>
              <span className="text-[10px] text-slate-400">{formatCurrency(day.amount)}</span>
            </div>)}
          </div>
        </div>
        <div className="card-premium p-6"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Care mix</p><h3 className="mb-4 mt-1 font-semibold text-slate-800">Patients by department</h3><div className="space-y-4">{departmentDist.map((dept, index) => <div key={dept.name}>
          <div className="mb-1 flex items-center justify-between"><span className="text-sm text-slate-600">{dept.name}</span><span className="text-sm font-bold text-slate-700">{dept.count}</span></div>
          <div className="h-2.5 overflow-hidden rounded-full bg-slate-100"><div className={`dept-progress-fill h-full rounded-full bg-gradient-to-r ${['from-brand-500 to-cyan-400', 'from-rose-500 to-orange-400', 'from-blue-500 to-sky-400', 'from-amber-500 to-yellow-400', 'from-emerald-500 to-teal-400', 'from-cyan-500 to-blue-400'][index % 6]}`} style={{ '--fill-width': `${(dept.count / maxDept) * 100}%`, width: `${(dept.count / maxDept) * 100}%`, animationDelay: `${index * 0.1}s` } as React.CSSProperties} /></div>
        </div>)}</div></div>
      </div>

      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="card-premium p-6"><div className="mb-4 flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Latest activity</p><h3 className="mt-1 font-semibold text-slate-800">Recent patients</h3></div><Users size={18} className="text-brand-500" /></div><div className="space-y-3">{recentPatients.map((patient) => <div key={patient.id} className="flex items-center gap-3 rounded-xl p-2 transition hover:bg-slate-50"><Avatar name={patient.name} /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-slate-700">{patient.name}</p><p className="text-xs text-slate-400">{patient.mrn} · {patient.department || 'General'}</p></div><span className={`badge ${patient.patient_type === 'IPD' ? 'badge-red' : 'badge-blue'}`}>{patient.patient_type}</span></div>)}</div></div>
